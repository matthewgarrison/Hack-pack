...
If \inline{edgeCount != numNodes-1} at the end, then the graph is not connected.
...
(This algorithm uses two \inline{Edge} objects to represent a single undirected edge, one for each vertex in the edge (ie. $v_1$ -$> v_2$ and $v_2$ -$> v_1$).)

If \inline{vertexCount != numNodes} at the end, then the graph is not connected.
